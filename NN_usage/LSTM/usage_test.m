[y1,fs] = audioread(['/Users/gongzhihao/Desktop/code/NN_usage/sound_data/Rain/1-63871-A.ogg']);
y1 = y1(:,1);
...
line(fre_y_location,fre_x_location, ...
        'Color','r', ...
        'LineStyle','--');
STATS = regionprops(block_set{21},'ConvexHull','Centroid','Area','Extent');
%STATS = regionprops(L,'ConvexHull','Centroid');

a = length(STATS);

figure();
imshow(result(fre_step*0+1:fre_step*1,time_step*2+1:time_step*3));
axis on;
hold on
for i = 1:a
    Draw_polygon(STATS(i).ConvexHull(:,1),STATS(i).ConvexHull(:,2));
